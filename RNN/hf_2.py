...
def test_softmax_loocv(seq, targets, n_updates=250, n_seq=182):
    """ Test RNN with softmax outputs. """
    length = len(seq)
...
def test_5_folder_cross_validation(seq, targets, n_updates=250, n_seq=182):
    length = len(seq)
    right, first, second, third = 0, 0, 0, 0
    m1=59/5
    m2=58/5
...
if __name__ == "__main__":
    ...
    train_lable=train_data[1]

    test_softmax_loocv(train_sample, train_lable, 11)

    # test_data=rtd.read_data()
    # test_sample=test_data[0]
    # test_lable=test_data[1]
    # for num in range(10,20):
    #     test_real_data(train_sample, train_lable,test_sample,test_lable,n_updates=num)
